 ...
 * @return State status.
 */
state_status_t set_lease_lock(cache_entry_t *entry, state_owner_t *owner,
				 ...
				 lock_type_t sle_type)
{
	...
	if (sle_type == LEASE_LOCK)
		return set_lease_lock(entry, owner, state, lock);
 ...
 * @param[in] lock     Lock description
 */
state_status_t put_lease_lock(cache_entry_t *entry,
			    state_owner_t *owner,
			    state_t *state,
			    fsal_lock_param_t *lock)
{
				   ...
				   lock_type_t sle_type)
{
	...
	if (sle_type == LEASE_LOCK)
		return put_lease_lock(entry, owner, state, lock);
